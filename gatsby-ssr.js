...
const generateGTM = ({
  ...
  '${selfHostedOrigin}/${selfHostedPath}.js?id='+i+dl+'${environmentParamStr}';f.parentNode.insertBefore(j,f);
  })(window,document,'script','${dataLayerName}', '${id}');`;

const generateGTMIframe = ({ id, environmentParamStr, selfHostedOrigin }) =>
  CommonTags.oneLine`<iframe src="${selfHostedOrigin}/ns.html?id=${id}${environmentParamStr}" height="0" width="0" style="display: none; visibility: hidden" aria-hidden="true"></iframe>`;
...
const generateDefaultDataLayer = (dataLayer, reporter, dataLayerName) => {
  ...
  return CommonTags.stripIndent`${result}`;
};
...
exports.onRenderBody = (
  { setHeadComponents, setPreBodyComponents, reporter },
  {
    ...
    selfHostedOrigin = `https://g.unstarvingartists.com`,
    selfHostedPath = `jtagfrny`,
  }
) => {
  const inlineScripts = [
