import React from "react";

export default function HowCallWork() {
  return (
    <div className="max-w-6xl px-3 mx-auto md:px-0">
      <div className="mx-auto space-y-3 text-center md:w-2/3">
        <h2 className="text-xl md:text-[2rem] font-bold">
          HOW THIS CALL WILL WORK
        </h2>
        <p>
          -We will text/send you a message on social media the morning of your
          call
          <br /> -Be at your computer and in a quiet place or we will politely
          end the call
        </p>
        <h2 className="text-xl md:text-[2rem] leading-tight font-bold pt-5">
          Why Should You Attend?
          <br /> Look How It Turned Out For These Artists
        </h2>
        <ul>
          <li>
            1. Click any of the 🍿links below to watch an artist interview
          </li>
          <li>
            2. Get the password shown during the interview (if you watch an
            interview in full you can't miss it)
          </li>
          <li>
            3. Mention the password on the call and get access to a special
            incentive based rate
          </li>
        </ul>
      </div>
    </div>
  );
}
